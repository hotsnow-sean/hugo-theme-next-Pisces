...
const search = () => {
  ...
  const getResultItems = (keywords) => {
    ...
    datas.forEach(({ title, content, url }) => {
      ...
      slicesOfContent.forEach((slice) => {
        resultItem += `<a href="${url.href}"><p class="search-result">${highlightKeyword(
          content,
          slice
        )}...</p></a>`;
      });
  ...
  const inputEventFunction = () => {
    ...
    if (keywords.length === 1 && keywords[0] === '') {
      ...
      const stats = `找到了 ${resultItems.length} 个搜索结果`;
  ...
  const fetchData = () => {
    const isXml = !CONFIG.localsearch.path.endsWith('json');
    fetch(CONFIG.localsearch.path)
  ...
  const highlightSearchWords = () => {
    ...
    const keywords = params.highlight
      ? params.highlight[0].split(/\+/).map(decodeURIComponent)
      : [];
